...
* [Cách xử lý các event do người dùng khởi tạo](/learn/responding-to-events)
* [Cách làm cho component "ghi nhớ" thông tin bằng state](/learn/state-a-components-memory)
...
## Phản hồi các event {/*responding-to-events*/}
...
Đọc **[Phản hồi các Event](/learn/responding-to-events)** để tìm hiểu cách thêm trình xử lý event.
...
Đọc **[State như một Snapshot](/learn/state-as-a-snapshot)** để tìm hiểu lý do tại sao state xuất hiện "cố định" và không thay đổi bên trong các trình xử lý event.

</LearnMore>

## Xếp hàng đợi một loạt các cập nhật state {/*queueing-a-series-of-state-updates*/}
...
[State như một Snapshot](/learn/state-as-a-snapshot) giải thích tại sao điều này xảy ra. Đặt state yêu cầu một re-render mới, nhưng không thay đổi nó trong code đã chạy. Vì vậy, `score` tiếp tục là `0` ngay sau khi bạn gọi `setScore(score + 1)`.
...
Đọc **[Xếp hàng đợi một loạt các cập nhật state](/learn/queueing-a-series-of-state-updates)** để tìm hiểu cách xếp hàng đợi một chuỗi các cập nhật state.

</LearnMore>

## Cập nhật các object trong state {/*updating-objects-in-state*/}
...
Nếu việc sao chép các object trong code trở nên tẻ nhạt, bạn có thể sử dụng một thư viện như [Immer](https://github.com/immerjs/use-immer) để giảm code lặp đi lặp lại:
...
Đọc **[Cập nhật các Object trong State](/learn/updating-objects-in-state)** để tìm hiểu cách cập nhật các object một cách chính xác.

</LearnMore>

## Cập nhật các array trong state {/*updating-arrays-in-state*/}
...
Nếu việc sao chép các array trong code trở nên tẻ nhạt, bạn có thể sử dụng một thư viện như [Immer](https://github.com/immerjs/use-immer) để giảm code lặp đi lặp lại:
...
Đọc **[Cập nhật các Array trong State](/learn/updating-arrays-in-state)** để tìm hiểu cách cập nhật các array một cách chính xác.

</LearnMore>

## Tiếp theo là gì? {/*whats-next*/}

Chuyển đến [Phản hồi các Event](/learn/responding-to-events) để bắt đầu đọc trang chương này từng trang một!
